...
class EducationalBackgroundActivity : AppCompatActivity() {
    ...
    private fun validateAndSave(): Boolean {
        ...
        val signature = findViewById<EditText>(R.id.signature)

        var isValid = true
        if (elemSchool.text.isBlank()) { elemSchool.error = "Required"; isValid = false }
        if (secSchool.text.isBlank()) { secSchool.error = "Required"; isValid = false }
        if (signature.text.isBlank()) { signature.error = "Required"; isValid = false }
        ...
        if (isValid) {
            getSharedPreferences("PDS_DATA", MODE_PRIVATE).edit {
                ...
                // College
                putString("colSchool", findViewById<EditText>(R.id.colSchool).text.toString())
                putString("colCourse", findViewById<EditText>(R.id.colCourse).text.toString())
                putString("colFrom", findViewById<EditText>(R.id.colFrom).text.toString())
                putString("colTo", findViewById<EditText>(R.id.colTo).text.toString())
                putString("colLevel", findViewById<EditText>(R.id.colLevel).text.toString())
                putString("colGrad", findViewById<EditText>(R.id.colGrad).text.toString())
                putString("colHonors", findViewById<EditText>(R.id.colHonors).text.toString())
                ...
                // Verification
                putString("signature", signature.text.toString())
                putString("date", findViewById<EditText>(R.id.date).text.toString())
            }
        }
        return isValid
    }
    ...
    private fun loadSavedData() {
        ...
        findViewById<EditText>(R.id.signature).setText(prefs.getString("signature", ""))
    }
}
